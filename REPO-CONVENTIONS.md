...
10. Register a third, separate deploy-only Entra app/service principal (`<server-name>-github-deploy` — deliberately *not* the `"<Server Name> MCP"` pattern from §5, since it carries no Graph scopes and isn't part of the runtime architecture) with a GitHub OIDC federated credential and **Website Contributor** scoped to just this server's Function App resource, then wire a `deploy-<server-name>` job into `.github/workflows/release-please.yml` gated on that server's release-please output. See `outlook-writeback/DEPLOYMENT.md`'s "CI/CD: automatic deploy via GitHub Actions" section for the exact commands and the GitHub secrets this needs — skip this step only if the new server is deliberately staying manual-deploy.
