...
- Confirm `mkdir -p` approach (§4.4) against both a OneDrive and a SharePoint library.
- Confirm eTag vs cTag semantics for `If-Match` on `/content` — these differ, and picking wrong yields either false 412s or no protection at all. **Verify separately on SharePoint**, where versioning may change the semantics.
...
3. **`mkdir -p` implementation** — pending Phase 0.
...
5. **eTag or cTag for `If-Match`, and does it differ on SharePoint?** Phase 0.
...
- Native AOT publish green in CI.
