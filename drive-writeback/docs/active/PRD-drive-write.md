...
**Content approval and minor versions.** Similar draft-state trap: the file is written but pending approval and not visible at the published version.
...
- Confirm `mkdir -p` approach (§4.4) against both a OneDrive and a SharePoint library. **Root-caused, fix applied, not yet confirmed against the live tenant.** A diagnostic test logging each created folder's actual `ParentReference.Path`/`Id` (plus a direct OneDrive-web check) confirmed folders created by the original path-based approach were landing at the drive root instead of nested where intended. Mechanism: colon-path addressing (`Items["root"].ItemWithPath(path)`) of a folder immediately after creating it is not reliable — Graph's path-resolution index can lag behind the item actually existing, and a `/children` POST against an unresolved colon-path silently lands at the drive root rather than erroring, instead of the clean 404 a naive implementation would expect. **Fix:** `CreateFolderPathAsync` now chains by the item `id` each create returns rather than re-deriving a path string from segment names — id-based addressing needs no path resolution, so it isn't exposed to this at all. This is a real Graph behavior worth carrying into Phase 1's `create_folder`/`get_item` design: **prefer id-based addressing over colon-path addressing for anything just created in the same call chain.** Two earlier diagnoses on this same failure (a "propagation lag on GET" theory, then a "colon-path GET itself is unreliable" theory) were both wrong — each was built on top of the real bug without having isolated it — and both were briefly written into this doc as confirmed findings before being retracted. Left in git history as a reminder to verify before writing "resolved." **Still untested on SharePoint.**
...
- Confirm `Files.ReadWrite.All` + `Sites.Read.All` is genuinely sufficient for all six use cases against a real library — i.e. that no write path demands `Sites.ReadWrite.All`. **This is the highest-value spike in Phase 0**; if it fails, §6 changes materially. **Partially resolved for OneDrive:** the eTag/cTag and item-ID spikes ran clean under only these two scopes, zero `403`s. The mkdir-p spike's failures were all `404`s, never a `403`, so nothing so far points at a scope problem specifically — but it hasn't actually completed successfully yet either (see above), so this isn't fully closed until it does. **Still open for SharePoint** — no team site available yet to exercise `Sites.Read.All`'s actual boundary (site/library resolution).
