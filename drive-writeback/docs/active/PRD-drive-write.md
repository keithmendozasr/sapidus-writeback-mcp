...
- Confirm `mkdir -p` approach (§4.4) against both a OneDrive and a SharePoint library. **Resolved for OneDrive, with a real gotcha:** the baseline "iterative per-segment create, 409 = already exists" approach works, but the natural way to fetch the already-existing item after a 409 — a colon-path `GET` on the item's own full path (`Items["root"].ItemWithPath(fullPath)`) — reproducibly 404'd immediately after the very 409 that proved the item exists, on a 4-segment-deep path, and a several-second bounded retry against that same call didn't clear it either. **Fix:** resolve the already-existing item by listing its *parent's* children and filtering by name instead (`parent.Children.GetAsync` with an OData `$filter=name eq '...'`) — the same mechanism that reliably created the tree in the first place, sidestepping whatever the colon-path `GET`'s issue is. Any implementation of `create_folder`'s conflict-already-exists path should use children-listing, not a direct path `GET`, to resolve the existing item. **Still untested on SharePoint** — a real gotcha found on OneDrive is reason to expect SharePoint needs the same verification, not skip it.
...
- Confirm `Files.ReadWrite.All` + `Sites.Read.All` is genuinely sufficient for all six use cases against a real library — i.e. that no write path demands `Sites.ReadWrite.All`. **This is the highest-value spike in Phase 0**; if it fails, §6 changes materially. **Resolved for OneDrive:** the mkdir-p, eTag/cTag, and item-ID spikes above all ran end-to-end under only these two scopes with zero `403`s anywhere. **Still open for SharePoint** — no team site available yet to exercise `Sites.Read.All`'s actual boundary (site/library resolution), so this isn't fully closed until that's tested.
...
3. ~~`mkdir -p` implementation~~ **Decided for OneDrive (§11):** iterative per-segment create, 409 = already exists — but resolve the already-existing item via parent children-listing + name filter, not a direct colon-path `GET`, which was found to be unreliable immediately after a 409. SharePoint still unverified.
