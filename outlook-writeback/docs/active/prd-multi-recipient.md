...
**Builds on:** `docs/archive/prd-create-update.md` ("v1" below) — this PRD is additive to and supersedes the recipient-related portions of v1 §6 (`create_draft`/`update_draft`'s input shape). It also supersedes one adjacent piece of v1 §6: `update_event`'s `attendees` update semantics and wire type, since fixing the underlying null-vs-empty ambiguity for email recipients means fixing it for the calendar side too — see §1 and §6 below. Every other section of v1 is unchanged; those sections are noted below with a one-line pointer back rather than restated.
...
- Let `create_draft` and `update_draft` accept more than one `To` recipient, plus optional `Cc` and `Bcc` recipients, each expressed as a native JSON array of address strings (`string[]?`) at the MCP tool boundary — the same array-of-`attendees` fix this PRD also makes to `update_event`, replacing the comma-separated-string convention v1 used for `attendees`.
...
- **Any change to `create_event`.** `create_event`'s `attendees` was already optional and has no prior state to preserve on create, so there's no "leave unchanged" state to define — only `update_event` needs the three-state semantics, and only `update_event` is touched. **This is a deliberately accepted schema inconsistency:** after this PRD ships, `attendees` is a comma-separated `string` on `create_event` but a `string[]` array on `update_event` — same field name, two different wire types, depending on which tool is called. See §13 Known Limitations.
...
- **Input shape change.** `attendees` changes from a single optional comma-separated `string` to a nullable array of strings (`string[]?`) — this is a real schema change to an already-shipped parameter, not just a semantics fix. See §13 Known Limitations for the compatibility note, and §3 for the resulting inconsistency with `create_event`'s unchanged `attendees` type.
...
- No change to `create_event`, `delete_event`, or any other calendar tool.
...
- Lands in `OutlookWriteback.Graph`, alongside the rest of the Graph-facing logic, unit-tested in `OutlookWriteback.Graph.Tests` the same way `BuildDraftMessage`/`BuildEvent` already are. This closes the coverage gap for `update_event`'s `attendees` and all of `create_draft`/`update_draft`'s recipient handling — it does **not** close it for `create_event`, whose `attendees` stays a comma-separated string with its inline, untested `Split` call untouched (§3 non-goal). That residual gap is accepted, not fixed, by this PRD.
...
- **Phase 1 — Multi-recipient support:** (Numbered from 1, not continuing v1's Phase 0–4 sequence — this is its own document with its own milestone list, not a later phase of the completed v1 PRD.)
  ...
  - Retype `update_event`'s `attendees` from `string` to `string[]?` in its MCP tool schema; leave `create_event`'s `attendees` untouched (still a comma-separated string) per the accepted inconsistency in §3/§13 — `Functions/CreateEventTool.cs` needs no changes in this phase.
...
**New limitation introduced by this PRD — `update_event`'s `attendees` breaks, not just changes meaning, for existing callers.** Today, `attendees` is a comma-separated string on `update_event`, and a blank one is a silent no-op. After this PRD ships, `attendees` is array-typed, so any existing caller (chat context, saved prompts, muscle memory) still passing a comma-separated string will fail MCP schema validation outright rather than silently misbehaving — the call errors instead of quietly clearing or no-op'ing. That's arguably a safer failure mode than v1's original silent-no-op ambiguity, but it's still a hard break with no deprecation window: the fix requires the reinterpretation and the retype together. Worth a one-line callout in the tool's MCP description (`Functions/UpdateEventTool.cs`) so a calling model doesn't assume the old string shape still works.

**New limitation: `attendees` has two different wire types depending on which tool is called.** `create_event`'s `attendees` stays a comma-separated `string` (§3 non-goal); `update_event`'s `attendees` becomes a `string[]`. Same field name, same conceptual data, different JSON types across two tools on the same server — an accepted inconsistency, not an oversight, but one a calling model could plausibly trip on by assuming the two tools share a schema. Worth the same one-line callout in both tools' MCP descriptions.
